...
export function StepRestoreParams({ onStarted }: { onStarted: (jobId: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-medium text-slate-900 dark:text-slate-100">Aus Sicherung wiederherstellen</h2>

      <div>
        <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">Sicherungsdatei (.tar.bz2)</label>
        <input type="file" accept=".bz2" onChange={handleFileChange} className="block w-full text-sm text-slate-700 dark:text-slate-300" />
        {file && !isTar && <p className="mt-1 text-xs text-red-600 dark:text-red-400">Bitte eine .tar.bz2-Sicherungsdatei auswählen.</p>}
      </div>

      <Field label="Servername (NetBIOS, max. 15 Zeichen)" error={newServerName && !SERVER_NAME_RE.test(newServerName) ? "1-15 Buchstaben/Ziffern/Bindestriche, kein Punkt." : undefined}>
        <TextInput value={newServerName} onChange={(e) => setNewServerName(e.target.value)} placeholder="SAMBA" />
      </Field>

      <Field label="IP-Adresse (optional, wird sonst automatisch erkannt)">
      ...
      <div className="flex justify-end">
        <Button onClick={submit} disabled={submitting || !file || !isTar || !SERVER_NAME_RE.test(newServerName)}>
          {submitting && <Spinner className="h-4 w-4" />} Wiederherstellen
        </Button>
      </div>
    </div>
  );
